...
- **The NN_CURRENT state vector is max-abs normalized to `[-1,1]`** at the end of `run_current_state` (nn_engine.cpp) before it feeds the seq/run/discard nets. Without this, the unbounded CURRENT logits (~30k–100k) dwarf the byte features (0–1) and starve the candidate-specific inputs of gradient signal. Preserve this normalization in any refactor.
