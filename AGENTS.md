...
- **The NN_CURRENT state vector is max-abs normalized to `[-1,1]`** in-engine (`normalize_state` in nn_engine.cpp, slot 0 only) before it feeds the seq/run/discard nets. Without this, the unbounded CURRENT logits (~30k–100k) dwarf the byte features (0–1) and starve the candidate-specific inputs of gradient signal. Preserve this normalization in any refactor.

## Behavioral invariants (verified in wasm_loader.js / game.js)
- Phase B executes only meld/appender moves with `score >= 0` (wasm_loader.js runTurn); negative-score plays are held for later turns. Don't change casually — training assumes it.
...
Full guide in `scripts/README.md`. Quick reference:
- **Start services** (from repo root): server `node server.js` in `buraco-server/` (port 8000); client `npm run dev -w buraco-client` (port 5173); bot `node bot.js` in `buraco-server/`. Outside Docker the bot needs `EUCHRE_SERVER_URL=http://localhost:8000` (and `MIGHTY_SERVER_URL=...`) because the `buraco-server` hostname doesn't resolve.
- **Detached processes** (survive a wrapping shell/tool timeout — `nohup`/`setsid` get killed): `python3 scripts/detach.py buraco-server server.log node server.js`. Find the PID via `/proc/<pid>/cmdline` (no `ps`/`pgrep` on locked-down hosts).
- **Headless browser test**: `node scripts/drive-euchre-browser.mjs` (starts a Euchre quick game, plays the human seat, stops when the trick-history panel renders, prints its grid DOM + a screenshot). **On this host the Chromium system libs, `fontconfig`, and fonts are already installed system-wide, so it runs with no extra env vars** (verified 2026-09-14).
- **Locked-down host (no apt/root)** — only if the host lacks the deps: `bash scripts/fetch-chromium-libs.sh` extracts the Chromium shared libs + DejaVu fonts into `scripts/.chromium-libs` and rewrites `scripts/fonts.conf`; then set `LD_LIBRARY_PATH`, `FONTCONFIG_FILE`, `FONTCONFIG_PATH` (see `scripts/README.md`). Deps to install on such a host: `curl`, `libglib2.0-0` (`libglib-2.0.so.0`), `fontconfig`+`libfontconfig1`+`fonts-dejavu-core`, plus the rest of the Chromium runtime (full list in `scripts/README.md` "System dependencies checklist").
